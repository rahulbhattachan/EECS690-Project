...
def main():
    ...
    # Modifications to use TextEnhancement function
    image = TextEnhancement(image, 1024)
    image_inverted = Image.fromarray(~np.array(image))

    image.save(f'{script_dir}/out/{name}_contrast.png')
    image_inverted.save(f'{script_dir}/out/{name}_contrast_inverted.png')
